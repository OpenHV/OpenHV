...
namespace OpenRA.Mods.HV.Traits
{
	...
	{
		readonly CarrierParent carrier;
		...
		public WithCarrierParentPipsDecoration(Actor self, WithCarrierParentPipsDecorationInfo info)
			: base(self, info)
		{
			carrier = self.Trait<CarrierParent>();
			pips = new Animation(self.World, info.Image);
			pipCount = Info.PipCount > 0 ? Info.PipCount : carrier.Info.Actors.Length;
		}
		...
		protected override IEnumerable<IRenderable> RenderDecoration(Actor self, WorldRenderer wr, int2 screenPos)
		{
			...
			var childrenInsideCount = carrier.GetChildrenInsideCount();
