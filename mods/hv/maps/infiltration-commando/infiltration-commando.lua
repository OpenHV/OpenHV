...
GroundPatrol = { "mgpod", "mgpod", "mgpod", "electricpod", "electricpod", "sniperpod", "sniperpod", "mbt", "mbt", "artillery", "stealthtank", "apc", "bike", "bike" }
...
SendPatroopers = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(2), function()
		Media.PlaySound("reinforcements_paratroopers_incoming.ogg")
	end)
...
SendAirstrikeWaves = function()
	...
	Trigger.AfterDelay((DateTime.Minutes(2) + DateTime.Seconds(20)) * AirstrikeDelayCoefficient, function()
		if not Human.IsObjectiveCompleted(InfiltrateRadarObjective) then
			...
			Media.PlaySound("airstrike_waves_started.ogg")
		end
	end)
	...
	Trigger.AfterDelay((DateTime.Minutes(16)) * AirstrikeDelayCoefficient, function()  -- Special wave onto the player's base
		if not Human.IsObjectiveCompleted(InfiltrateRadarObjective) then
			...
			Media.PlaySound("airstrike_waves_player.ogg")
		end
	end)
...
RevealPowerPlants = function()
	...
	Trigger.AfterDelay(DateTime.Minutes(1) + DateTime.Seconds(40), function()
		...
		Media.PlaySound("com_quarters_revealed.ogg")
		Trigger.AfterDelay(300, function()  -- wait for 5 secs to send a reminder
			...
			Media.PlaySound("capture_radar_reminder.ogg")
		end)
	end)
	...
	Trigger.AfterDelay(DateTime.Minutes(4) + DateTime.Seconds(20), function()
...
Tick = function()
	...
	if #radar2 == 0 and #radar3 == 1 and not Human.IsObjectiveCompleted(SaveColonyObjective) then  -- If the enemy has no more radar, and the player has one, complete "infiltrate" and "save colony" objectives
		...
		Media.PlaySound("captured_radar_dome.ogg")
	end
	...
	if #enemyharbors == 0 and not Human.IsObjectiveCompleted(DestroyHarborFacilities) then
		...
		Media.PlaySound("neutralized_enemy_harbor.ogg")
	end
	...
	if enemyheadquarters == 0 and not Human.IsObjectiveCompleted(SaveColonyObjective) then
		...
		Human.MarkCompletedObjective(SaveColonyObjective)
		Human.MarkCompletedObjective(InfiltrateRadarObjective)
		Media.PlaySound("annihilated_yuruki_headquarters.ogg")
	end
...
function UpdateGameStateText()
	...
	if percentage < DifficultyPercentage * 1.1 and not SentReminder then
		...
		Media.PlaySound("reminder_low_percentage.ogg")
	end
...
WorldLoaded = function()
	...
	if Difficulty == "easy" then
		...
		GroundPatrol = { "mgpod", "mgpod", "electricpod", "mbt", "artillery", "apc", "bike" }
		AirPatrol = { "gunship", "jet" }
		NavalSmallPatrol = { "lightboat.patrol", "lightboat.patrol", "lightboat.patrol" }
	end
	if Difficulty == "hard" then
		NavalPatrol = { "lightboat", "lightboat", "lightboat", "torpedoboat", "torpedoboat", "torpedoboat", "lightningboat", "lightningboat", "carrier", "carrier" }
		GroundPatrol = { "mgpod", "mgpod", "mgpod", "electricpod", "electricpod", "sniperpod", "bomberpod", "bomberpod", "mbt", "mbt", "mbt", "artillery", "stealthtank", "apc", "bike", "bike", "lightningtank" }
		AirPatrol = { "gunship", "gunship", "gunship", "gunship", "jet", "jet", "jet" }
		NavalSmallPatrol = { "lightboat.patrol", "lightboat.patrol", "lightboat.patrol", "lightboat.patrol", "lightboat.patrol", "lightningboat.patrol", "lightningboat.patrol" }
	end
